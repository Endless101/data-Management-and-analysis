CREATE INDEX article for (a:article) on a.key;
CREATE INDEX proceeding for (p:proceeding) on p.key; 
CREATE INDEX inproceeding for (ip:inproceeding) on ip.key;
CREATE INDEX author for (a:author) on a.key;
CREATE INDEX conference for (c:conference) on c.key;
CREATE INDEX journal for (j:journal) on j.key;
CREATE INDEX editor for (e:editor) on e.key;
